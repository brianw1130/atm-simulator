...
class TestGenerateStatement:
    ...
    async def test_statement_with_default_days(
        self, mock_pdf, db_session: AsyncSession
    ):
    ...
    @patch("src.atm.services.statement_service.generate_statement_pdf")
    async def test_statement_with_custom_days(
        self, mock_pdf, db_session: AsyncSession
    ):
        account = await _seed_account_with_customer(db_session)
    ...
    @patch("src.atm.services.statement_service.generate_statement_pdf")
    async def test_statement_with_date_range(
        self, mock_pdf, db_session: AsyncSession
    ):
        account = await _seed_account_with_customer(db_session)
        today = date.today()
        ...
        result = await generate_statement(
            db_session, account.id, start_date=start, end_date=today
        )
    ...
    @patch("src.atm.services.statement_service.generate_statement_pdf")
    async def test_includes_transactions_in_range(
        self, mock_pdf, db_session: AsyncSession
    ):
        account = await _seed_account_with_customer(
            db_session, balance_cents=515_000
        )
    ...
    @patch("src.atm.services.statement_service.generate_statement_pdf")
    async def test_opening_closing_balance_calculation(
        self, mock_pdf, db_session: AsyncSession
    ):
    ...
    @patch("src.atm.services.statement_service.generate_statement_pdf")
    async def test_file_path_contains_masked_account(
        self, mock_pdf, db_session: AsyncSession
    ):
        account = await _seed_account_with_customer(db_session)
        result = await generate_statement(db_session, account.id)
    ...
    @patch("src.atm.services.statement_service.generate_statement_pdf")
    async def test_pdf_generator_called_with_account_info(
        self, mock_pdf, db_session: AsyncSession
    ):
        account = await _seed_account_with_customer(
            db_session, balance_cents=525_000
        )
    ...
    @patch("src.atm.services.statement_service.generate_statement_pdf")
    async def test_session_id_passed_to_audit(
        self, mock_pdf, db_session: AsyncSession
    ):
        account = await _seed_account_with_customer(db_session)
        result = await generate_statement(
    ...
    @patch("src.atm.services.statement_service.generate_statement_pdf")
    async def test_debit_reduces_opening_balance(
        self, mock_pdf, db_session: AsyncSession
    ):
        ...
        account = await _seed_account_with_customer(
            db_session, balance_cents=100_000
        )
        await _seed_transaction(
            db_session,
            account.id,
